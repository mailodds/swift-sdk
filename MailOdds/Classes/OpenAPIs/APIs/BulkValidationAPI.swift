...
open class BulkValidationAPI {
     ...
     */
    @discardableResult
    open class func retryJob(jobId: String, apiResponseQueue: DispatchQueue = MailOddsAPI.apiResponseQueue, completion: @escaping ((_ data: RetryJob200Response?, _ error: Error?) -> Void)) -> RequestTask {
        return retryJobWithRequestBuilder(jobId: jobId).execute(apiResponseQueue) { result in
            switch result {
            case let .success(response):
                completion(response.body, nil)
            case let .failure(error):
    ...
    open class func retryJobWithRequestBuilder(jobId: String) -> RequestBuilder<RetryJob200Response> {
        var localVariablePath = "/v1/jobs/{job_id}/retry"
        let jobIdPreEscape = "\(APIHelper.mapValueToPathItem(jobId))"
        let jobIdPostEscape = jobIdPreEscape.addingPercentEncoding(withAllowedCharacters: .urlPathAllowed) ?? ""
        localVariablePath = localVariablePath.replacingOccurrences(of: "{job_id}", with: jobIdPostEscape, options: .literal, range: nil)
        let localVariableURLString = MailOddsAPI.basePath + localVariablePath
        ...
        let localVariableRequestBuilder: RequestBuilder<RetryJob200Response>.Type = MailOddsAPI.requestBuilderFactory.getBuilder()
